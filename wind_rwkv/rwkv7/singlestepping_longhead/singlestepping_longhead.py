...
class RWKV7_longhead(th.autograd.Function):
    ...
    def forward(ctx, q,w,k,v,a,b,s0):
        ...
        if not th.compiler.is_compiling():
            assert hasattr(th.ops.wind_singlestepping_longhead, 'forward'), 'Requires a load kernel from load_singlestepping_longhead(head_size)'
        ...
        if any(i.requires_grad for i in [w,q,k,v,a,b,s0]):
            ...
            s = sa = None
        th.ops.wind_singlestepping_longhead.forward(w,q,k,v,a,b, s0,y,s,sa,sT)
    ...
    def backward(ctx, dy, dsT):
        ...
        th.ops.wind_singlestepping_longhead.backward(w,q,k,v,a,b, dy,s,sa,dsT, dw,dq,dk,dv,da,db,ds0)
        return dq,dw,dk,dv,da,db,ds0

def attn_singlestepping_longhead(r,w,k,v,a,b, s0 = None):
    B,T,H,C = w.shape
    if s0 is None: s0 = th.zeros(B,H,C,C, dtype=th.bfloat16,device=w.device)
    return RWKV7_longhead.apply(r,w,k,v,a,b, s0)

def load_singlestepping_longhead(head_size, batchsz_times_heads_estimate = 8*64):
    if hasattr(th.ops.wind_singlestepping_longhead, 'forward'): return
    ...
    load(name="wind_singlestepping_longhead", sources=[os.path.join(path,'singlestepping_longhead.cu'), os.path.join(path,'singlestepping_longhead.cpp')], is_python_module=False, verbose=False, extra_cuda_cflags=CUDA_FLAGS)
    assert hasattr(th.ops.wind_singlestepping_longhead, 'forward')

def attn_singlestepping_longhead_wrap(r,w,k,v,a,b, head_size):
    B,T,HC = w.shape
    C = head_size
    H = HC//C
    r,w,k,v,a,b = [i.view(B,T,H,C) for i in [r,w,k,v,a,b]]
    s0 = th.zeros(B,H,C,C, dtype=th.bfloat16,device=w.device)
    return attn_singlestepping_longhead(r,w,k,v,a,b,s0)[0].view(B,T,HC)
